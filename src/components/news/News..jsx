...
function News({ categoryToShow }) {
  ...
  return (
    <>
      <div className="bg-electrification_1">
        <div>
          <img
        ...
        <section className="container mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {filteredBlogs.length > 0 && (
              <>
                <div className="col-span-1 lg:col-span-2 bg-white rounded-lg shadow-md relative">
                  <Link to={`/blog/${filteredBlogs[0].id}`}>
                    <div className="relative">
                      <img
                        src={`${fileURL}/${filteredBlogs[0].image}`}
                        alt="News Image 1"
                        className="w-full h-full max-h-[500px] min-h-[500px] object-cover rounded-t-lg"
                      />
                      ...
                      <div className="bg-white rounded-lg shadow-md relative">
                        <div className="relative">
                          <img
                            ...
                            className="w-full h-full max-h-[230px] min-h-[230px] object-cover rounded-t-lg"
                          />
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4 text-white">
                            <div className="flex flex-col h-full justify-end">
                              <h3 className="text-xl font-semibold mb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {currentBlogs.map((blog) => (
              <Link key={blog.id} to={`/blog/${blog.id}`}>
                <div className="bg-white rounded-lg shadow-md overflow-hidden h-[350px] group">
                  <div className="overflow-hidden">
                    <img
                      src={`${fileURL}/${blog.image}`}
                      alt={blog.title}
                      className="w-full h-[168px] max-h-full object-fill rounded-t-lg transition-transform duration-500 ease-in-out group-hover:scale-110 "
                    />
                  </div>
